return {
	...
	config = function()
		...
		require('competitest').setup {
			...
			template_file = {
				cpp = "~/.config/nvim/lua/template/temp.cpp"
			},
			evaluate_template_modifiers = true,
			date_format = "%c",
			received_files_extension = "cpp",
			-- received_problems_path = "$(HOME)/Competitive Programming/$(JUDGE)/$(CONTEST)/$(PROBLEM).$(FEXT)",
			received_problems_path = "$(HOME)/MyStuff/CP/$(JUDGE)/Problems/$(PROBLEM).$(FEXT)",
			received_problems_prompt_path = false,
			received_contests_directory = "$(HOME)/MyStuff/CP/$(JUDGE)/$(CONTEST)",
